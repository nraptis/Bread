...
OUTPUT_FILE="${ROOT_DIR}/run_maze_perseverance_tests_report.txt"
...
{
  echo "[INFO] run_maze_perseverance_tests started at $(date)"
  echo "[INFO] output file: ${OUTPUT_FILE}"
  echo "[INFO] knobs from tests/common/Tests.hpp:"
  rg -n "TEST_LOOP_COUNT|MAZE_TEST_DATA_LENGTH|TEST_SEED_GLOBAL|REPEATABILITY_ENABLED" \
  ...
  echo "[STEP] Building maze_perseverance_tests"
  cmake --build "${BUILD_DIR}" --target maze_perseverance_tests -j"$(detect_jobs)"
  echo

  echo "[STEP] Running maze_perseverance_tests"
  "${BUILD_DIR}/maze_perseverance_tests"
  echo

  echo "[PASS] run_maze_perseverance_tests finished at $(date)"
} | tee "${OUTPUT_FILE}"
