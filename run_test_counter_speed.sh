...
OUTPUT_FILE="${ROOT_DIR}/run_counter_speed_report.txt"
...
{
  echo "[INFO] run_test_counter_speed started at $(date)"
  ...
  echo "[STEP] Building benchmark_counter_speed_comparison"
  cmake --build "${BUILD_DIR}" --target benchmark_counter_speed_comparison -j"$(detect_jobs)"
  echo

  echo "[STEP] Running benchmark_counter_speed_comparison"
  "${BUILD_DIR}/benchmark_counter_speed_comparison"
  echo

  echo "[PASS] run_test_counter_speed finished at $(date)"
} | tee "${OUTPUT_FILE}"
