...
OUTPUT_FILE="${ROOT_DIR}/run_game_speed_benchmarks_output.txt"
...
{
  echo "[INFO] run_game_speed_benchmarks started at $(date)"
  echo "[INFO] output file: ${OUTPUT_FILE}"
  echo "[INFO] knobs from tests/common/Tests.hpp:"
  rg -n "TEST_LOOP_COUNT|GAME_TEST_DATA_LENGTH|TEST_SEED_GLOBAL|BENCHMARK_ENABLED|REPEATABILITY_ENABLED|GAME_REPEATABILITY_FULL_ENABLED" \
  ...
  "${BUILD_DIR}/benchmark_games_speed_repeatability"
  echo

  echo "[STEP] Running benchmark_minigames_speed_9216"
  "${BUILD_DIR}/benchmark_minigames_speed_9216"
  echo

  echo "[PASS] run_game_speed_benchmarks completed at $(date)"
} | tee "${OUTPUT_FILE}"
