...
int main() {
  if (!bread::tests::config::REPEATABILITY_ENABLED) {
    std::cout << "[SKIP] maze edge connected disabled by Tests.hpp\n";
    return 0;
  }
  ...
  std::cout << "[INFO] maze edge connected"
            << " loops=" << aLoops
            << " bytes=" << aSeedLength
  ...
  for (int aLoop = 0; aLoop < aLoops; ++aLoop) {
    ...
    bread::tests::maze_quality::FillInput(&aInput, aLoop, 0xC2B2AE35U);
    ...
    if (((aLoop + 1) % 10) == 0 || (aLoop + 1) == aLoops) {
      std::cout << "[INFO] maze edge connected progress"
                << " loop=" << (aLoop + 1) << "/" << aLoops
  ...
  std::cout << "[PASS] maze edge connected passed"
            << " loops=" << aLoops
            << " bytes=" << aSeedLength
