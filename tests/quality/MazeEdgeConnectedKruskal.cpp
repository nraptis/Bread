...
int main() {
  if (!bread::tests::config::REPEATABILITY_ENABLED) {
    std::cout << "[SKIP] maze edge connected kruskal disabled by Tests.hpp\n";
    return 0;
  }
  ...
  std::cout << "[INFO] maze edge connected kruskal"
            << " loops=" << aLoops
            << " bytes=" << aSeedLength
  ...
  for (int aLoop = 0; aLoop < aLoops; ++aLoop) {
    ...
    bread::tests::maze_quality::FillInput(&aInput, aLoop, 0xA4093822U);
    ...
    if (((aLoop + 1) % 10) == 0 || (aLoop + 1) == aLoops) {
      std::cout << "[INFO] maze edge connected kruskal progress"
                << " loop=" << (aLoop + 1) << "/" << aLoops
  ...
  std::cout << "[PASS] maze edge connected kruskal passed"
            << " loops=" << aLoops
            << " bytes=" << aSeedLength
