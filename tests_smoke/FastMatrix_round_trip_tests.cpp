...
#include "src/Tables/matrices/FastMatrix.hpp"

namespace {

void FillPattern(peanutbutter::matrices::FastMatrix* pMatrix) {
  unsigned char* aData = pMatrix->Data();
  for (int aIndex = 0; aIndex < peanutbutter::matrices::FastMatrix::kSize; ++aIndex) {
    aData[aIndex] = static_cast<unsigned char>(aIndex);
  }
}

bool MatchBaseline(const peanutbutter::matrices::FastMatrix& pMatrix) {
...
int main() {
  peanutbutter::matrices::FastMatrix aMatrix;
