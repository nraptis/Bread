...
triage_measure_failure() {
  local log_file="$1"
  echo
  echo "[TRIAGE] predictability benchmark failed."
  if [[ -f "${log_file}" ]]; then
...
set +e
"${BUILD_DIR}/benchmark_counter_predictability_all" 2>&1 | tee "${MEASURE_LOG}"
...
echo "[PASS] Predictability release measurement completed"
