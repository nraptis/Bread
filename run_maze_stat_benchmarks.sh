...
OUTPUT_FILE="${ROOT_DIR}/run_maze_stat_benchmarks_output.txt"
...
{
  echo "[INFO] run_maze_stat_benchmarks started at $(date)"
  ...
  echo "[STEP] Building benchmark_maze_stats"
  cmake --build "${BUILD_DIR}" --target benchmark_maze_stats -j"$(detect_jobs)"
  echo

  echo "[STEP] Running benchmark_maze_stats"
  "${BUILD_DIR}/benchmark_maze_stats"
  echo

  echo "[PASS] run_maze_stat_benchmarks completed at $(date)"
} | tee "${OUTPUT_FILE}"
