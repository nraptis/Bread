...
OUTPUT_FILE="${ROOT_DIR}/run_game_stat_benchmarks_output.txt"
...
{
  echo "[INFO] run_game_stat_benchmarks started at $(date)"
  echo "[INFO] output file: ${OUTPUT_FILE}"
  echo "[INFO] knobs from tests/common/Tests.hpp:"
  rg -n "TEST_LOOP_COUNT|GAME_TEST_DATA_LENGTH|TEST_SEED_GLOBAL|BENCHMARK_ENABLED" "${ROOT_DIR}/tests/common/Tests.hpp" || true
  ...
  echo "[STEP] Running benchmark_games_stats"
  "${BUILD_DIR}/benchmark_games_stats"
  echo

  echo "[PASS] run_game_stat_benchmarks completed at $(date)"
} | tee "${OUTPUT_FILE}"
