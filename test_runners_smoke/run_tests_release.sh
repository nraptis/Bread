...
echo "[STEP] Running test suites (Release)"
CTEST_LOG="${BUILD_DIR}/ctest_release_output.log"
set +e
ctest --test-dir "${BUILD_DIR}" -L "smoke" --output-on-failure 2>&1 | tee "${CTEST_LOG}"
...
echo "[PASS] Release test suites completed"
