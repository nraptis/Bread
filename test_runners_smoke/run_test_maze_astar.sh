...
OUTPUT_FILE="${ROOT_DIR}/run_test_maze_astar_output.txt"
...
{
  echo "[INFO] run_test_maze_astar started at $(date)"
  ...
  echo "[STEP] Building maze_path_finding"
  cmake --build "${BUILD_DIR}" --target maze_path_finding -j"$(detect_jobs)"
  echo

  echo "[STEP] Running maze_path_finding"
  "${BUILD_DIR}/maze_path_finding"
  echo

  echo "[PASS] run_test_maze_astar finished at $(date)"
} | tee "${OUTPUT_FILE}"
